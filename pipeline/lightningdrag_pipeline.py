...
class LightningDragPipeline(StableDiffusionPipeline):
                ...
                for m in torch_dfs(down_block):
                    if isinstance(m, BasicTransformerBlock):
                        ...
                        processor = PointEmbeddingAttnProcessor(
                            ...
                            use_norm=self.use_norm_attn_processor,
                        ).to(device, dtype)
    ...
    def prepare_point_embeddings(
        ...
        handle_embeddings, target_embeddings = point_embedding(
            handle_disk_map, target_disk_map
        )

        # repeat if needed
        if handle_embeddings[0].shape[0] < batch_size:
            assert (
                batch_size % handle_embeddings[0].shape[0] == 0
            ), "shape mismatch with batch size"
            num_img = handle_embeddings[0].shape[0]
            handle_embeddings = [
                h.repeat(batch_size // num_img, 1, 1, 1) for h in handle_embeddings
            ]
            target_embeddings = [
                t.repeat(batch_size // num_img, 1, 1, 1) for t in target_embeddings
            ]
    ...
    def __call__(
        ...
        if self.image_encoder is not None:
            ...
            clip_image = (
                self.feature_extractor(images=pil_image, return_tensors="pt")
                .pixel_values[0]
                .unsqueeze(0)
            )
            clip_image_embeds = self.image_encoder(
                clip_image.to(prompt_embeds.dtype).to(self.image_encoder.device)
            ...
            negative_image_prompt_embeds = negative_image_prompt_embeds.repeat(
                num_images_per_prompt, 1, 1
            )
            prompt_embeds = torch.cat(
                [prompt_embeds_tuple[0], image_prompt_embeds], dim=1
            )
            negative_prompt_embeds = torch.cat(
                [prompt_embeds_tuple[1], negative_image_prompt_embeds], dim=1
            )
        ...
        if self.fusion_blocks == "midup":
            ...
            raise NotImplementedError(
                f"fusion blocks {self.fusion_blocks} not implemented"
            )
        ...
        appr_encoder_hidden_state = negative_prompt_embeds[
            :, : negative_prompt_embeds.shape[1] - self.num_ip_tokens, :
        ]  # only use text part
        if do_classifier_free_guidance:
            ref_image_latents = torch.cat([ref_image_latents] * 2, dim=0)
            appr_encoder_hidden_state = torch.cat(
                [appr_encoder_hidden_state] * 2, dim=0
            )
        ...
        if do_classifier_free_guidance:
            handle_embeddings = [
                torch.cat([torch.zeros_like(emb), emb], dim=0)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if guidance_scale_decay == "none":
                    ...
                    cur_guidance_scale_points = (guidance_scale_points - 1.0) * (
                        1.0 - i / len(timesteps)
                    ) ** 2 + 1
                else:
                ...
                if num_guidance_steps is None or (
                    ...
                    noise_pred = noise_pred_uncond + cur_guidance_scale_points * (
        ...
        return StableDiffusionPipelineOutput(
            images=image, latents=latents, nsfw_content_detected=has_nsfw_concept
        )
